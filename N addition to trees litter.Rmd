...
expected_k_rate<-gather(expected_K,plot, 'expected_k', 'plot1', 'plot2', 'plot3')
litter<-full_join(dat_m,expected_k_rate, by = c("species","plot"))

litter$mer<- litter$K/litter$expected_k
#View(litter)
...
model_no_3way <- update(mod4, . ~ . - Nitrogen_added:tree_quality:mesh_size)
anova(model_no_3way)
anova(mod4,model_no_3way)
...
Nitrogen addition, litter quality, litterbag mesh size, and interactive effects between litter quality and N addition significantly influenced the litter decomposition rate. Among these factors, nitrogen addition level and mesh size exhibited independent roles in the litter mixing effect, while a significant interactive effect was observed between litter quality and nitrogen addition. Macrofauna (body size > 2 mm) effects were more sensitive to nitrogen addition, particularly under high nitrogen addition treatments. In the presence of soil macrofauna, nitrogen addition accelerated the decomposition of low-quality litter and enhanced litter mixing effects, with these positive effects increasing at higher N addition rates. In contrast, for high-quality litter, low-level N addition had a greater promoting effect than high-level N addition.  These findings indicate that nitrogen deposition effect during litter decomposition depends on litter substrate quality and decomposer groups.